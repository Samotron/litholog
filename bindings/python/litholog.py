...
class SecondaryConstituent:
    ...
    def to_dict(self) -> Dict[str, str]:
        ...
        return {"amount": self.amount, "soil_type": self.soil_type}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "secondary_constituents": [
                sc.to_dict() for sc in self.secondary_constituents
            ],
        }
...
class LithologLibrary:
    ...
    def __init__(self, library_path: str = "./liblitholog.so"):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load litholog library at {library_path}: {e}"
            ) from e
    ...
    def parse(self, description: str) -> Optional[SoilDescription]:
        ...
        if c_result.contents.weathering_grade >= 0:
            result.weathering_grade = WeatheringGrade(
                c_result.contents.weathering_grade
            )
        ...
        if c_result.contents.secondary_constituents_count > 0:
            for i in range(c_result.contents.secondary_constituents_count):
                ...
                result.secondary_constituents.append(
                    SecondaryConstituent(amount, soil_type)
                )

        # Copy strength parameters
        if (
            c_result.contents.has_strength_parameters
            and c_result.contents.strength_parameters
        ):
            ...
            result.strength_parameters = StrengthParameters(
                param_type, value_range, sp.confidence
            )
...
def parse(
    description: str, library_path: str = "./liblitholog.so"
) -> Optional[SoilDescription]:
    """Parse a geological description string.
            ...
            try:
                ...
                if result:
                    ...
                    if result.strength_parameters:
                        ...
                        print(
                            f"  Range: {sp.value_range.lower_bound} - {sp.value_range.upper_bound}"
                        )
